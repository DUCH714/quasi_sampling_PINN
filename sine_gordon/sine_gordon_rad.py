"""
Sine-Gordon Equation Solver using Physics-Informed Neural Networks (PINNs) with RAD.
...
def compute_RAD(model, ob_x, frozen_para):
...
def train(key):
    ...
    for j in range(ite * N_epochs):
        if j % N_epochs == 0:
            ...
            rad_points = x_in_set.sample(N_interior*50, keys[0])
            rad_points = jnp.concatenate([rad_points,
                                    vmap(right_hand_side, (0, None, None))(rad_points, vec_c, dim,
                                                                                 ).reshape(-1, 1)],-1)
            ...
            ob_x = random.choice(keys[1], rad_points, shape=(N_interior,), p=err_eq_normalized,replace=False)

            x_b, y_b = x_b_set.sample(N_b, keys[2])
