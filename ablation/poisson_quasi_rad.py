"""
Poisson Equation Solver using Physics-Informed Neural Networks (PINNs) with RAD sampling on low-discrepancy sequence.

This module implements a PINN-based solution for the Poisson equation:
    ∇²u = f

Usage:
    Training mode:  python poisson_quasi_rad.py --mode train --network mlp --dim 100
    Evaluation mode: python poisson_quasi_rad.py --mode eval --network mlp --dim 100
"""
...
def train(key):
    ...
    for j in range(ite * N_epochs):
        if j % N_epochs == 0:
            ...
            rad_points = x_train_set
            rad_points = jnp.concatenate([rad_points,
                                    vmap(right_hand_side, (0, None, None))(rad_points, alpha, dim,
                                                                                 ).reshape(-1, 1)],-1)
